...
def Barker_proposal(x, partial_logpi_x, step_size, noise='normal'):
    if noise == 'bimodal':
        # z ~ Normal(mean = sigma, sd = 0.1 * sigma)
        z = np.random.normal(loc=step_size, scale=0.3*step_size, size=len(partial_logpi_x))
        
        # Acceptance probability for each component: 1 / (1 + exp(-grad * z))
        acceptance_prob = 1.0 / (1.0 + np.exp(-partial_logpi_x * z))
...
def Barker(target, n_iter, x_init, step_size=1, noise='normal'):
    ...
    for i in range(n_iter):
        # Proposal state
        y = Barker_proposal(x, partial_logpi_x, step_size, noise)
