...
sns.set_palette(
    sns.color_palette(wes.film_palette("The Life Aquatic with Steve Zissou"))
)
...
def traceplot(X, w=6, h=4, n_col=2, overlay=True):
    ...
    if overlay:
        ...
        fig, axes = plt.subplots(
            nrows=n_row, ncols=n_col, figsize=(w * n_col, h * n_row)
        )
...
def acfplot(X, lags=50, w=6, h=4, n_col=2, overlay=True):
    ...
    if overlay:
        ...
        for i in range(n_dim):
            ...
            sns.scatterplot(
                x=range(0, lags + 1), y=acf_values, ax=ax, size=0.5, legend=False
            )
...
def rosenbrock_pairplot(X, n1, n2, sampler_names=None):
    column_labels = ["X_{1}"]
    ...
    if isinstance(X, list):
        ...
        grid = sns.pairplot(
            combined_df, hue="Sampler", diag_kind="kde", plot_kws={"s": 5, "alpha": 0.2}
        )
